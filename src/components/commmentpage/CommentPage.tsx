...
export default function CommentPage() {
  ...
  const { commentData, isLoading, error } = useGetCommentQuery(
    { id: currentId },
    {
  ...
  const { commentsData, isFetchingNextPage, hasNextPage, fetchNextPage } =
    useGetCommentCommentsInfiniteQuery(
      isUUID(currentId) ? currentId : skipToken,
      {
        selectFromResult: (result) => ({
          ...result,
          commentsData: result.data?.pages
  ...
  return (
    <StyledMain>
      <StyledDefault>
        {isLoading ? (
          <StyledErrorMessage>Loading Comment...</StyledErrorMessage>
        ) : error ? (
          <StyledErrorMessage>Can't Find Comment.</StyledErrorMessage>
        ) : commentData ? (
          <>
            <StyledBack />
          ...
          <StyledErrorMessage>No Comment Found!</StyledErrorMessage>
        )}
      </StyledDefault>
    </StyledMain>
  );
