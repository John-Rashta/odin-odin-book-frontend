...
import CommentCreate from "../partials/comment/CommentCreate";
...
export default function PostPage() {
  ...
  const { postData, isLoading, error } = useGetPostQuery(
    { id: currentId },
    {
  ...
  const {
    ...
    isUUID(currentId) ? currentId : skipToken,
    {
      selectFromResult: (result) => ({
        ...result,
  ...
  return (
    <StyledMain>
      <StyledDefault>
        {isLoading ? (
          <StyledErrorMessage>Loading Post...</StyledErrorMessage>
        ) : error ? (
          <StyledErrorMessage>Can't Find Post.</StyledErrorMessage>
        ) : postData ? (
          <>
            <StyledBack />
            <PostProfile post={postData} modalFunc={editFunction} />
            {isUUID(myId) && (
              <StyledForm placeName="Comment here..." postid={postData.id} />
            )}
            {commentsLoading ? (
              <StyledErrorMessage>Loading Comments!</StyledErrorMessage>
            ) : commentsError ? (
              <StyledErrorMessage>Failed Loading Comments!</StyledErrorMessage>
              ...
              <StyledErrorMessage>No Comments Yet!</StyledErrorMessage>
            )}
          </>
        ) : (
          <StyledErrorMessage>No Post Found!</StyledErrorMessage>
        )}
      </StyledDefault>
