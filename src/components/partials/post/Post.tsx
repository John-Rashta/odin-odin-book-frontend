...
export default function Post({
  ...
  info: FullPostInfo & Likes & YourLike & OwnCommentsCount;
  modalFunc?: ModalStartFunction;
}) {
    ...
    <StyledContainer className="clickOption postOption" data-postid={info.id}>
      <div>
        <StyledImage
          ...
          data-userid={info.creator.id}
          src={info.creator.customIcon?.url || info.creator.icon.source}
          alt=""
        />
      </div>
      <MainContainer>
        <TopContainer>
          ...
          <TopRightContainer>
            <StyledEdited>{info.edited ? "Edited" : ""}</StyledEdited>
            {typeof modalFunc === "function" && (
              <ShowOptions
                myId={myId}
                id={info.creatorid}
                textStuff={{
        ...
        <BottomContainer>
          <StyledFlex>
            <StyledCounts>{info.ownCommentsCount}</StyledCounts>
            <MessageSquare />
          </StyledFlex>
          <BottomRightContainer>
            <StyledCounts>{info.likesCount}</StyledCounts>
            <LikeButton
              myId={myId}
              likesInfo={info.likes}
              clickFunction={(e) => {
                ///e.currentTarget.disabled = true;
                changeLike({
                  id: info.id,
                  action:
                    info.likes && info.likes.length > 0 ? "REMOVE" : "ADD",
                })
                  .unwrap()
                  .finally(() => {
