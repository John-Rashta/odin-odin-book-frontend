...
import CommentsDisplayButtons from "../buttons/CommentsDisplayButtons";
import LoadMore from "../buttons/LoadMore";
import LikeButton from "../buttons/LikeButton";
import ShowOptions from "../options/ShowOptions";
...
export default function Comment({
  ...
  const { commentsData, isFetchingNextPage, hasNextPage, fetchNextPage } =
    useGetCommentCommentsInfiniteQuery(showComments ? comment.id : skipToken, {
  ...
  return (
    <StyledComment>
      <StyledMainComment
        className="clickOption commentOption"
          ...
          <StyledExtraBottom>
            {showReply && (
              <CommentCreate
                ...
                changeCreate={() => setShowReply(false)}
              />
            )}
            <CommentsDisplayButtons
              depth={depth}
              id={comment.id}
              count={comment.ownCommentsCount}
              ...
              <StyledDropDown>
                <ClickWrapper>
                  {commentsData.map((ele) => {
                    return (
                      <Comment depth={depth + 1} key={ele.id} comment={ele} />
                    );
                  })}
                </ClickWrapper>
                <StyledLoad
